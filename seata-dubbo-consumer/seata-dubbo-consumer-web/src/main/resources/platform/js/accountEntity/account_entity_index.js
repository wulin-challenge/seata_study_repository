...
$(function(){
	colNames= ["ID","账户昵称","账户总金额","账户用户名","监区","监狱","创建日期","修改时间"];
    colModel= [
        { name: "id", index:"id",hidden: true},
	    { name: "nickName", index:"nickName", align:"center", sortable: true,searchoptions :{sopt:['eq','cn']}},
	    { name: "totalAmount", index:"totalAmount", align:"center", sortable: true,searchoptions :{sopt:['eq','cn']}},
	    { name: "username", index:"username", align:"center", sortable: true,searchoptions :{sopt:['eq','cn']}},
	...
	var jqGridAddOptions={tableId:"#commonList",colNames:colNames,colModel:colModel,url:requestURL,
			pagerId:"#commonPager",title:"账户列表",sortname:"gmtCreate"
	};
	//初始化列表
...
function showCommonDetailWindow(operation,data,pageData){
	...
	var options = {
			formId:commonDetailFormId,
			windowId:windowDivId,
			windowParam:{title:"账户详细信息",width:$(window).width()*0.9,height:$(window).height()*0.9},
			data:data,
			pageData:pageData
	};
